...
if __name__ == "__main__":
    (train_texts, train_labels), (val_texts, val_labels) = (util.load_gdpr_dataset('sample.txt','labels.txt'))
    ...
    model = train.train_sequence_model(train_texts=train_texts,
                               ...
                               pool_size= 1,
                               top_k= 20000,
                               ) #load_saved_weights="saved.h5" Add this parameter if you want to load a saved h5 file

    preds = model.predict(val_texts)
    util.eval(preds,val_labels)